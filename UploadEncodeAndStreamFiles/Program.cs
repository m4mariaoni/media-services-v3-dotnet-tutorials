    ...
    public class Program
    {
            ...
            string fileToUpload)
        {
            ...
            try
            {
                ...
                //blob.Upload(fileToUpload);


                // Path to the directory to upload
                string uploadPath = Directory.GetCurrentDirectory() + "\\Videos";
                ...
                int count = 0;
                var aa = Directory.GetFiles(uploadPath);
                ...
                foreach (string filePath in Directory.GetFiles(uploadPath))
                {
                    ...
                    BlobClient blob2 = container.GetBlobClient(fileName);

                    // Add the upload task to the queue
                    tasks.Enqueue(blob2.UploadAsync(filePath, options));
                    count++;
                }
            ...
            //await blob.UploadAsync(fileToUpload, upOptions);

            // Open this file and upload it to blob
            //using (FileStream fileStream = File.OpenRead(Path.GetFileName(fileToUpload)))
            //{
            //    await blob.UploadAsync(fileStream,upOptions);
            //}
